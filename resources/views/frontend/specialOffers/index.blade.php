        ...
        <div class="col-md-12">
            @can('special_offer_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.special-offers.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $specialOffer->id }}">
                                        ...
                                        <td>
                                            @can('special_offer_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.special-offers.show', $specialOffer->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('special_offer_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.special-offers.edit', $specialOffer->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('special_offer_delete')
                                                <form action="{{ route('frontend.special-offers.destroy', $specialOffer->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('special_offer_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.special-offers.massDestroy') }}",
