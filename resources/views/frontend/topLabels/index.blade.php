        ...
        <div class="col-md-12">
            @can('top_label_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.top-labels.create') }}">
                            ...
                            <tbody>
                                @foreach($topLabels as $key => $topLabel)
                                    <tr data-entry-id="{{ $topLabel->id }}">
                                        ...
                                        <td>
                                            @can('top_label_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.top-labels.show', $topLabel->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('top_label_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.top-labels.edit', $topLabel->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('top_label_delete')
                                                <form action="{{ route('frontend.top-labels.destroy', $topLabel->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('top_label_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.top-labels.massDestroy') }}",
