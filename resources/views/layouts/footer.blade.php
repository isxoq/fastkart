...
<div class="modal fade theme-modal view-modal" id="detailAjaxModal" tabindex="-1" aria-labelledby="exampleModalLabel"
     aria-hidden="true">
...
@section("scripts")
    <script>
        ...
        $(document).on("click", '#detailAjaxBtn', function (e) {

            let id = $(this).attr("data-id")
            e.preventDefault()
            $.ajax("/api/product/detail-ajax/" + id, {
                success: function (data) {
                    // console.log(data)
                    $('#detailModalContent').html(data)
                    $('#detailAjaxModal').modal("show")
                }
            })
        })
