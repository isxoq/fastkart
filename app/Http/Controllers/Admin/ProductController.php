...
class ProductController extends Controller
{
    ...
    public function store(StoreProductRequest $request)
    {
        ...
        if ($request->input('card_photo', false)) {
            $product->addMedia(storage_path('tmp/uploads/' . basename($request->input('card_photo'))))->toMediaCollection('card_photo');
        }
    ...
    public function update(UpdateProductRequest $request, Product $product)
    {
        ...
        if ($request->input('card_photo', false)) {
            if (! $product->card_photo || $request->input('card_photo') !== $product->card_photo->file_name) {
                if ($product->card_photo) {
                    $product->card_photo->delete();
                }
                $product->addMedia(storage_path('tmp/uploads/' . basename($request->input('card_photo'))))->toMediaCollection('card_photo');
            }
        } elseif ($product->card_photo) {
            $product->card_photo->delete();
        }
